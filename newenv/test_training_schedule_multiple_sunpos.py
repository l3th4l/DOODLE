...
from newenv_rl_test import HelioField, rotate_normals_batch
...
def train_batched(batch_size=25, 
    ...
    anti_spillage_factor = 0000.0
    alignment_factor = 10000.0  # Factor to scale alignment loss
    ...
    for step in range(steps):
        ...
        #alignment loss 
        B = sun_positions.shape[0] if sun_positions.dim() > 1 else 1
        normal_vectors = action.view(batch_size, -1)
        errors = noisy_field.error_angles_mrad.unsqueeze(0).expand(B,-1,-1)
        flats = normal_vectors.reshape(-1,3)
        errs_flat = errors.reshape(-1,2)

        #ideal normals 
        ideal_normals = noisy_field.calculate_ideal_normals(sun_positions).reshape(-1, 3)
        ...
        alignment_loss = calculate_angles_mrad(actual, ideal_normals).mean()

        loss = (anti_spillage_factor * loss_bound
                #+ anti_spillage_factor * loss_bound * decay_factor
                + alignment_factor * alignment_loss
        ...
        if step % 50 == 0 or step == steps - 1:
            print(f"Step {step:3d} | Loss: {final_loss:.6f}  "
                  f"[dist {loss_dist.item():.6f}, alignment {alignment_loss:.4f}, bound {loss_bound:.6f}, mse {mse_loss.item():.6f}] | LR: {lr:.6e}")
