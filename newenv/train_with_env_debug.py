...
from test_environment_debug import HelioEnv  # your multi-error env
...
class PolicyNet(nn.Module):
    def init_weights_high_entropy(self):
        ...
        # Optional: Randomize final layer's output more strongly
        if hasattr(self, 'head'):
            for m in self.head.modules():
                if isinstance(m, nn.Linear):
                    nn.init.normal_(m.weight, mean=0.0, std=3.0)  # boost std
                    nn.init.normal_(m.bias, mean=0.0, std=3.0)
    ...
    def __init__(self,
        ...
        #self.init_weights_high_entropy()
...
def rollout(env, policy, k, T, device, use_mean=False):
    ...
    B = env.batch_size * env.batch_size
...
def train_and_eval(args, plot_heatmaps_in_tensorboard = True, return_best_mse = True):
    ...
    for step in range(args.steps + pretrain_steps):
        ...
        for i in range(args.num_batches):
            ...
            if True: #(args.num_batches * step + i < (pretrain_steps)) or parts['alignment_loss'] > best_alignment_loss: #last_alignment_loss:
                eff_step = step
                decay = max(0, (cutoff - eff_step) / cutoff)

                loss = alignment_f * (parts['alignment_loss'] * decay + parts['masked_alignment_loss'] * (1-decay) ) * 10000
            ...
            #    loss = anti_spill * parts['bound']
                ...
                loss  = (mse_f * parts['mse']*(1-decay+1e-5)
                        ...
                        + parts['masked_alignment_loss'] * 100)
            ...
            if (args.num_batches * step + i > (warmup_steps + pretrain_steps)) : #(not args.disable_scheduler) and 
                if args.scheduler == "plateau":
                    sched.step(parts['alignment_loss'].item())
                elif args.scheduler == "cyclic":
                    sched.step()
                elif args.scheduler == "exp":
                    sched.step()
            ...
            if (step % 50 == 0) or step == args.steps-1:
                scatter3d_vectors(
                    ...
                    origin_position= heliostat_pos.mean(dim=0).view([-1,]),
                    target_position= targ_pos.view([-1,]), 
                ...
                scatter3d_vectors(
                    monitor['reflected_rays'].view([-1, 3]).detach().cpu().numpy(), 
                    monitor['all_alignment_loss'].view([-1,]).detach().cpu().numpy(),
                    origin_position= heliostat_pos.mean(dim=0).view([-1,]),
                    target_position= targ_pos.view([-1,]), 
                    html_file=f'./monitors_debug/step_{step}/batch_{i}_r_alignment.html')
                ...
                scatter3d_vectors(
                    ...
                    origin_position= heliostat_pos.mean(dim=0).view([-1,]),
                    target_position= targ_pos.view([-1,]), 
                    html_file=f'./monitors_debug/step_{step}/batch_{i}_alignment.html')
        # ------------------------------------------------------------
        ...
        writer.add_scalar("loss/alignment", parts['alignment_loss'], step)
