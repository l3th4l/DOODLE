...
from smooth_adam_opt import SmoothedAdam
...
def rollout(field_ref, field_noisy, policy, k, T,
    ...
    for _ in range(T):
        ...
        if hx is not None:
            normals, hx = policy(net_img.detach(), aux.detach(),
                                 (hx[0].detach(), hx[1]))
        else:
            normals, hx = policy(net_img.detach(), aux.detach(), hx)

        img, action = field_noisy.render(
            sun_pos, normals.flatten(1)), normals.flatten(1)

        hist = torch.roll(hist, -1, dims=1)
        hist[:, -1] = img

    # compute losses
    target_img = field_ref.render(
        sun_pos, ideal_normals.flatten(1).detach())
...
def train_and_eval(args, plot_heatmaps_in_tensorboard=True):
    ...
    for step in range(args.steps):
        ...
        def closure():
            # NO zero_grad() or backward() here
            parts, pred_imgs, _ = rollout(
                ref_field, noisy_field, policy,
                args.k, args.T,
            ...
            recorded['pred_imgs'] = pred_imgs
            recorded['loss']      = loss
            return loss

        # SmoothedAdam.step will call closure() repeatedly, use autograd.grad internally
        loss = opt.step(closure)

        parts     = recorded['parts']
        pred_imgs = recorded['pred_imgs']
    ...
    parts, pred_img, target_img = rollout(
        ref_field, noisy_field, policy,
        args.k, args.T,
        heliostat_pos, targ_pos, targ_norm,
        targ_area, distance_maps, sun_pos, dev)
